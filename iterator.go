...
type nodeIteratorState struct {
	hash    ogTypes.Hash // Hash of the node being iterated (nil if not standalone)
	node    Node         // Trie node being iterated
	parent  ogTypes.Hash // Hash of the first full ancestor node (nil if current is the root)
...
func (it *nodeIterator) Hash() ogTypes.Hash {
	if len(it.stack) == 0 {
		return EmptyHash
	}
...
func (it *nodeIterator) Parent() ogTypes.Hash {
	if len(it.stack) == 0 {
		return EmptyHash
	}
...
func (it *nodeIterator) peek(descend bool) (*nodeIteratorState, *int, []byte, error) {
	...
	for len(it.stack) > 0 {
		...
		if ancestor.HashKey() == EmptyHash.HashKey() {
...
func (st *nodeIteratorState) resolve(tr *Trie, path []byte) error {
	if hash, ok := st.node.(HashNode); ok {
		...
		st.node = resolved
		st.hash = ogTypes.BytesToHash(hash)
	}
	return nil
}

func (it *nodeIterator) nextChild(parent *nodeIteratorState, ancestor ogTypes.Hash) (*nodeIteratorState, []byte, bool) {
...
func (it *differenceIterator) Hash() ogTypes.Hash {
	return it.b.Hash()
}

func (it *differenceIterator) Parent() ogTypes.Hash {
	return it.b.Parent()
}
...
func (it *differenceIterator) Next(bool) bool {
	...
	for {
		...
		case 0:
			...
			hasHash := it.a.Hash().HashKey() == EmptyHash.HashKey()
...
func (it *unionIterator) Hash() ogTypes.Hash {
	return (*it.items)[0].Hash()
}

func (it *unionIterator) Parent() ogTypes.Hash {
...
func (it *unionIterator) Next(descend bool) bool {
	...
	for len(*it.items) > 0 && ((!descend && bytes.HasPrefix((*it.items)[0].Path(), least.Path())) || compareNodes(least, (*it.items)[0]) == 0) {
		...
		if skipped.Next(skipped.Hash().HashKey() == EmptyHash.HashKey()) {
			it.count++
